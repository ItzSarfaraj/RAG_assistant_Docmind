...
import { postSSE } from "../services/sseFetch";

// NOTE: This page renders inside DashboardLayout's <main className="flex-1
// overflow-hidden">, which already owns h-screen at the app root and hands
// this page the correct remaining height via flexbox. Use h-full here, not
// h-screen — h-screen would re-claim a full 100vh on top of the layout's
// navbar/header height and clip content at the bottom.
const PAGE_HEIGHT_CLASS = "h-full";
...
function NotesPage() {
  ...
  useEffect(() => {
    const loadNote = async () => {
      try {
        ...
        if (data.note?.explanationLevel)
          setExplanationLevel(data.note.explanationLevel);
        if (data.note?.noteStructure) setNoteStructure(data.note.noteStructure);
        if (typeof data.note?.faithfulToVideo === "boolean")
          setFaithfulToVideo(data.note.faithfulToVideo);
      ...
      await postSSE(
        ...
        (event) => {
          if (event.type === "progress") {
            setRegenProgress(event);
          } else if (event.type === "complete") {
            result = event.note;
          } else if (event.type === "error") {
            throw new Error(event.message || "Failed to regenerate notes.");
          }
        },
      );

      if (!result?._id) throw new Error("No note ID returned.");
  ...
  if (loading) {
    return (
      <div className={`flex ${PAGE_HEIGHT_CLASS} items-center justify-center bg-[#F7F4EC]`}>
        <div className="text-center">
          <LoaderCircle
            size={26}
            className="mx-auto animate-spin text-[#BD7B24]"
          />
  ...
  if (error && !note) {
    return (
      <div className={`flex ${PAGE_HEIGHT_CLASS} items-center justify-center bg-[#F7F4EC] px-6`}>
        <div className="max-w-sm text-center">
          ...
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="mt-5 flex mx-auto items-center gap-2 rounded-lg bg-[#22201A] px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#3A362C]"
          >
            <ArrowLeft size={14} />
            Go Back
          </button>
  ...
  return (
    <div className={`flex ${PAGE_HEIGHT_CLASS} overflow-hidden bg-[#F7F4EC] text-[#22201A]`}>
      <NotesSidebar
        open={leftOpen}
        ...
        <header className="print-hide sticky top-0 z-20 flex h-16 items-center justify-between border-b border-[#E6E1D3] bg-white/95 px-4 backdrop-blur sm:px-6">
          <div className="flex min-w-0 items-center gap-2">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="rounded-lg p-2 text-[#75705F] hover:bg-[#F7F4EC]"
              title="Go back"
            >
              <ArrowLeft size={17} />
            </button>
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[9px] text-[#8A8473]">
          ...
          <button
            ...
            className="flex shrink-0 items-center gap-2 rounded-lg bg-[#22201A] px-3 py-2 text-xs font-semibold text-white hover:bg-[#3A362C]"
            title="Save as PDF"
          >
            <Download size={14} />
